...
T Set_union(T s, T t)
{
    if (s == NULL) {
        ...
        /* if both of s and t are not NULL,
         * then get a copy of set s first, then
         * insert each member in set t into the new
         * copy set.
         * */
...
T Set_minus(T s, T t)
{
    if (t == NULL) {
        ...
        set = Set_new(Arith_min(s->size, t->size), s->cmp, s->hash);
        {
            int i;
            struct member *q;
            for (i = 0; i < t->size; i++)
